...
export const FHIRClient = {
  ...
  getResource: async (resourceType, resourceId) => {
    if (USE_MOCK_DATA) {
      try {
        ...
        // If the data is a single object, wrap it in an array for uniform handling
  ...
  sendToExternalFhirServer: async (resourceType, data) => {
    ...
    try {
      ...
      console.log(`Successfully sent ${resourceType} to FHIR server:`, result);
      return result;
    } catch (error) {
   ...
   * 
   * NEED TO FIGURE OUT HOW TO GENERATE ID
   * 
   */
